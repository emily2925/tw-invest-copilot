...
def fetch_overnight_intraday() -> pd.DataFrame:
    """最近一次夜盤「整個交易時段內」的逐分鐘走勢（15:00 開始到隔天05:00）。

    這是 TAIFEX 官網首頁「夜盤台股期貨近月份走勢圖」本身在用的公開 API
    （objId=13），免費不用登入，資料是目前最新一次夜盤（若現在是非交易時段，
    就是最近一次已結束的夜盤）逐分鐘價格，不是好幾天的收盤價序列。
    """
    resp = requests.get(
        TAIFEX_QUOTES_API,
        params={"objId": "13", "_": str(int(time.time() * 1000))},
        headers={"User-Agent": "Mozilla/5.0"},
        timeout=10,
    )
    resp.raise_for_status()
    data = resp.json()
    if not data:
        raise RuntimeError("抓不到夜盤逐分鐘走勢資料")

    df = pd.DataFrame(data)
    df["price"] = df["price"].astype(float)
    return df.rename(columns={"price": "Close", "time": "Time"})[["Time", "Close"]]
